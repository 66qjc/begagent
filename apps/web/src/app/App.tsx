...
export function App(_props: AppProps) {
  ...
  const pursuit = workspace.pursuits[0]
  const pursuitStage = pursuit?.stage ?? 'discovered'
  ...
  const next = nextCommand(pursuitStage, pursuit?.id ?? '', client)
...
function stageTitle(stage: PursuitStage): string {
  const labels: Record<PursuitStage, string> = {
    discovered: '岗位已导入', qualified: '岗位判断已完成', growth_plan: '挑战路径已确认',
    evidence_sprint: '正在完成岗位证据冲刺', materials_ready: '岗位版简历已更新', application_awaiting_approval: '申请等待你的确认',
    application_submitted: '岗位申请已提交', hr_active: 'HR 沟通正在进行', completed: '求职任务已完成',
  }
  return labels[stage]
}

function stageDescription(stage: PursuitStage): string {
  const labels: Record<PursuitStage, string> = {
    discovered: '系统已装载确认事实、求职偏好与沟通边界。', qualified: '五层岗位判断已经形成，可选择直接投递或补强证据。',
    growth_plan: '系统正在创建跨 Agent 任务包。', evidence_sprint: '面试成长 Agent 主责，真实成果完成后才会写入简历。',
    materials_ready: '新证据已经进入职业素材库，求职执行 Agent 重新接管。', application_awaiting_approval: '外部动作尚未发生，审批记录将进入执行轨迹。',
